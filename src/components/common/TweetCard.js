...
function TweetCard({ tweet }) {
    ...
    const [retweetUserDetails, setRetweetUserDetails] = useState()
    ...
    useEffect(() => {
        const init = async () => {
            ...
            if (tweet.retweetedUser && !retweetedByUser) {
                const d = await getUserDetails(tweet.retweetedUser)
                setRetweetUserDetails(d)
            }
        ...
        tweetUserDetails !== null && (
            <>
                ...
                <div className="d-flex flex-column justify-content-start align-items-start">
                    {retweetUserDetails && (
                        <span className="small" style={{ marginLeft: '4rem' }}>
                            Retweeted by{' '}
                            <Link
                                to={`/app/${retweetUserDetails.username}`}
                                className="react-link--none"
                            >
                                {retweetUserDetails.username}
                            </Link>
                        </span>
                    )}
                    <div
